        ...
        for (name, value) in v.items():
            ...
            if ObjVal._mode == MODE_VALUE:
                if ObjVal.TYPE == TYPE_OPEN and isinstance(ObjVal._typeref, ASN1RefClassIntern):
                    ...
                    r.append('({0}, {1})'.format(qrepr(name), value_to_defin(value, ObjValTr, Gen)))
                else:
                    r.append('({0}, {1})'.format(qrepr(name), value_to_defin(value, ObjVal, Gen)))
                ...
                if ObjVal.TYPE == TYPE_OPEN and isinstance(ObjVal._typeref, ASN1RefClassIntern):
                    ...
                    for p in ObjVal._typeref.ced_path[1:]:
                        ObjValTr = ObjValTr.get_cont()[p]
                    r.append('({0}, {1})'.format(qrepr(name), set_to_defin(value, ObjValTr, Gen)))
                else:
                    r.append('({0}, {1})'.format(qrepr(name), set_to_defin(ASN1Set(value), ObjVal, Gen)))
            else:
                ...
                r.append('({0}, {1})'.format(qrepr(name), value._pyname))
...
class GoGenerator(_Generator):
    ...
    def gen(self):
        ...
        for mod_name in [mn for mn in GLOBAL.MOD if mn[:1] != '_']:
            ...
            for attr in ('_obj_', '_type_', '_set_', '_val_', '_class_', '_param_'):
                self.wil('var {0} = []string{{'.format(attr))
    ...
    def gen_type(self, Obj, compts=False):
        ...
        self.wil('{0} := Asn{1}{{{2}}}'.format(Obj._pyname,
    ...
    def gen_set(self, Obj):
        ...
        self.wil('{0}.Val := {1}'.format(Obj._pyname, set_to_defin(ASN1Set(Obj._val), Obj, self)))
    ...
    def gen_val(self, Obj):
        ...
        #
        # now generate the set of values
        self.wil('{0}._val = {1}'.format(Obj._pyname, value_to_defin(Obj._val, Obj, self)))
    ...
    def gen_type_int(self, Obj):
        ...
        if Obj._cont:
            ...
            for a in Obj._cont.items():
                  self.wil('{0}.Cont.Add{1}'.format(extract_charstr(Obj._pyname), a))
    ...
    def gen_type_enum(self, Obj):
        ...
        if Obj._cont:
            for a in Obj._cont.items():
                  ...
                  self.wil('{0}.Cont.Add("{1}",{2})'.format(Obj._pyname, r,a[1]))
            #self.wil('{0}._cont = ASN1Dict({1!r})'.format(Obj._pyname, list(Obj._cont.items())))
            if Obj._ext is None:
                ...
                if len(Obj._ext) > 0:
                    self.wil("DATA_HERE")
                self.wil('{0}.Ext = {1!r}'.format(Obj._pyname, Obj._ext))
        # value constraint
        self.gen_const_val(Obj)
    ...
    def gen_type_bitstr(self, Obj):
        ...
        if Obj._cont:
            ...
            for a in Obj._cont.items():
                  r = extract_charstr(a[0])[0]
                  self.wil('{0}.Cont.Add("{1}",{2})'.format(Obj._pyname, r))
            #self.wil('{0}._cont = ASN1Dict({1!r})'.format(Obj._pyname, list(Obj._cont.items())))
        # value constraint
        self.gen_const_val(Obj)
        # size constraint
    ...
    def gen_type_choice(self, Obj):
        ...
        if Obj._cont is not None:
            ...
            for a in links:
                  self.wil('{0}.Cont.Add({1},{2})'.format(Obj._pyname, qrepr(a), links[a]))
            ...
            if Obj._ext is None:
                ...
                if len(Obj._ext) > 0:
                    self.wil("DATA_HERE")
                for a in Obj._ext:
                    self.wil('{0}.Ext.Add({1})'.format(Obj._pyname, a))
        # value constraint
        self.gen_const_val(Obj)
    ...
    def gen_type_seqof(self, Obj):
        ...
        if Obj._cont is not None:
            ...
            self.wil('{0}._cont = {1}'.format(Obj._pyname, Cont._pyname))
        # value constraint
        self.gen_const_val(Obj)
        # size constraint
        self.gen_const_sz(Obj)
    ...
    def gen_type_class(self, Obj):
        ...
        if Obj._cont is not None:
            ...
            #self.wil('{0}._cont = ASN1Dict(['.format(Obj._pyname))
            for name in links:
                self.wil('\t{0}.Cont.Add({1}, {2})'.format(Obj._pyname, qrepr(name), links[name]))
    ...
    def gen_const_sz(self, Obj):
        ...
        if Consts_size:
            ...
            for er in Ssz._er:
                if er.lb is None:
                    er.lb = 0
            self.wil('{0}._const_sz = {1}'.format(Obj._pyname, set_to_defin(Ssz, SzProxy, self)))
    ...
    def gen_const_val(self, Obj):
        ...
        if Consts_val:
            ...
            self.wil('{0}._const_val = {1}'.format(Obj._pyname, set_to_defin(Sval, Obj, self)))
    ...
    def gen_const_table(self, Obj):
        ...
        if Consts_tab:
            ...
            self.wil('{0}._const_tab = {1}'.format(Obj._pyname, link_name))
            # define the @ identifier
            if Const['at'] is None:
                self.wil('{0}._const_tab_at = nil'.format(Obj._pyname))
            else:
#                self.wil('{0}._const_tab_at = ASN1Ref{{"{1}","{2}"}}'.format(Obj._pyname, tuple(Const['at'])))
                self.wil('{0}._const_tab_at = ASN1Ref{{"{1}","{2}"}}'.format(Obj._pyname, Const['at'][0],Const['at'][1]))
             # define the table object identifier
            try:
                self.wil('{0}._const_tab_id = {1}'.format(Obj._pyname, qrepr(Obj._typeref.ced_path[-1])))
            except:
                asnlog('WNG, {0}.{1}: unavailable table constraint ident, not compiling it'\
                       .format(self._mod_name, Obj._name))
                self.wil('{0}._const_tab_id = None')
